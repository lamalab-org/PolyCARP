...
|---|---|---:|
| `processed_data.csv` | The cleaned reaction dataset that feeds `02-reactivity-prediction/copol_prediction/create_data_split.py` → `artifacts/data_splits/` → `artifacts/model_bundle/`. | 7,582 measurement rows = 3,791 unique reactions |
...
`grouped_by_unique_monomer_pairs.csv` is fully regenerable from `processed_data.csv` via `02-reactivity-prediction/copol_prediction/build_grouped_dataset.py`; it's committed as a convenience for citation and as the artefact the Zenodo deposit archives.

## How this differs from `02-reactivity-prediction/copol_prediction/processed_data.csv`

The repo has **two `processed_data.csv` files**; they serve different consumers:

| Path | Consumer | Rows | Reactions |
|---|---|---:|---:|
| `02-reactivity-prediction/copol_prediction/processed_data.csv` | The live API's nearest-neighbour lookup pool (`api/baseline_lookup.py`) | 4,969 | 3,791 |
| `02-reactivity-prediction/copol_prediction/paper_dataset/processed_data.csv` | Input to `create_data_split.py` (this file) | 7,582 | 3,791 |
...
```
paper_dataset/processed_data.csv       02-reactivity-prediction/copol_prediction/processed_data.csv
...
2. `python 02-reactivity-prediction/copol_prediction/build_grouped_dataset.py --source paper_dataset/processed_data.csv --output paper_dataset/grouped_by_unique_monomer_pairs.csv` — refresh the grouped view; its row count is the new dataset-size number for the paper.
3. `python 02-reactivity-prediction/copol_prediction/create_data_split.py` — recut `artifacts/data_splits/`.
4. `python 02-reactivity-prediction/copol_prediction/train_final_model.py` — retrain into `artifacts/model_bundle/`.
5. `python 02-reactivity-prediction/copol_prediction/reproduce_paper_metrics.py` — capture the new metrics into `artifacts/paper_metrics.json` and update the paper's `tab:train_test_voting_performance`.
