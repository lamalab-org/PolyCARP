...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description="Train final copolymerization prediction model")
    parser.add_argument(
        "--output-dir",
        type=str,
...
def prepare_data(config):
    ...
    if config.get("remove_specialized", False):
        if "specialized_filter" in df_train.columns:
            ...
            df_train = df_train[df_train["specialized_filter"] != "specialized"].reset_index(
                drop=True
            )
            removed_count = before_count - len(df_train)
            print(f"Removed {removed_count} specialized datapoints from training set")
            print(
                f"  Training set: {len(df_train)} samples ({df_train['reaction_id'].nunique()} groups)"
            )
        else:
    ...
    if prune_path and os.path.exists(prune_path):
        try:
            ...
            if "reaction_id" in df_prune.columns:
                ...
                df_train = df_train[
                    ~df_train["reaction_id"].astype(str).isin(prune_ids)
    ...
    if config["add_negative_data"]:
        neg_path = os.path.join(
            _SCRIPT_DIR, "filter", "artificial_datapoints", "processed_combined_augmented.csv"
        )
...
def fit_and_save_validation_calibration(
    *,
    ...
    df_train_lookup_pool = df_train_lookup_pool.dropna(
        subset=features + ["r_product_class"]
    ).reset_index(drop=True)
...
def save_model(model_info, holdout_results, config):
    ...
    specialized_removed = (
        split_info.get("remove_specialized_from_test", False) if split_info else False
    )
...
def save_all_metrics_to_file(model, df_train, df_test, features, output_dir, config):
    ...
    train_precision_per_class = precision_score(
        y_train, y_train_pred, average=None, zero_division=0, labels=[0, 1, 2]
    )
    ...
    test_precision_per_class = precision_score(
        y_test, y_test_pred, average=None, zero_division=0, labels=[0, 1, 2]
    )
    ...
    with open(metrics_file, "w") as f:
        ...
        for i, class_label in enumerate([0, 1, 2]):
            f.write(
                f"Class {class_label:<6} {train_precision_per_class[i]:<15.6f} {train_recall_per_class[i]:<15.6f} {train_f1_per_class[i]:<15.6f}\n"
            )
        f.write("\n")
        ...
        for i, class_label in enumerate([0, 1, 2]):
            f.write(
                f"Class {class_label:<6} {test_precision_per_class[i]:<15.6f} {test_recall_per_class[i]:<15.6f} {test_f1_per_class[i]:<15.6f}\n"
            )
        f.write("\n")
...
def evaluate_voting_on_test_set(model, df_train_lookup_pool, df_test, features, output_dir):
    ...
    print(
        f"Voting coverage: {payload['voting']['coverage']:.3f} ({payload['voting']['agree_n']} retained)"
    )
...
def save_train_test_performance_latex_table(
    ...
    df_train_lookup_pool = df_train_lookup_pool.dropna(subset=lookup_required_cols).reset_index(
        drop=True
    )
    ...
    try:
        ...
        if args.latex_table:
            ...
            analyze_model.create_latex_per_class_table(
                predictor, output_dir, confidence_threshold=args.confidence_threshold
            )
        ...
        if cal_ok:
            print(
                f"  ✓ Calibration curves written under: {output_dir}/ (calibration_curves.png/pdf)"
            )
        else:
            print(
