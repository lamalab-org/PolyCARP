...
def create_split(remove_specialized=False, test_size=0.2, val_size=0.1):
    ...
    if os.path.exists(specialized_path):
        ...
        if (
            "llm_specialized_filter" in df_classified.columns
            and "original_source" in df_classified.columns
        ):
    ...
    print(
        f"After NaN removal: {len(df_clean)} samples (removed {len(df_filtered) - len(df_clean)})"
    )
...
if __name__ == "__main__":
    args = parse_args()
    create_split(
        remove_specialized=args.remove_specialized, test_size=args.test_size, val_size=args.val_size
    )
