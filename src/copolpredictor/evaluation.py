...
def save_results_with_confidence(
    ...
    if len(predictions_df) == len(df_clean):
        ...
        print(
            f"Warning: Predictions length ({len(predictions_df)}) doesn't match data length ({len(df_clean)})"
        )
    ...
    med_conf = (predictions_df["confidence_score"] > 0.6) & (
        predictions_df["confidence_score"] <= 0.8
    )
    low_conf = predictions_df["confidence_score"] <= 0.6

    confidence_stats = {
        "high_confidence_accuracy": (
            correct_predictions[high_conf].mean() if high_conf.sum() > 0 else 0
        ),
        "medium_confidence_accuracy": (
            correct_predictions[med_conf].mean() if med_conf.sum() > 0 else 0
        ),
        "low_confidence_accuracy": (
            correct_predictions[low_conf].mean() if low_conf.sum() > 0 else 0
        ),
