...
sys.path.insert(0, str(PROJECT_ROOT / "02-reactivity-prediction" / "copol_prediction"))
...
def evaluate_model_on_dataset(model, df_test, features, dataset_name):
    ...
    for feat in model_features:
        print(
            f"   {feat}: min={X_test[feat].min():.3f}, max={X_test[feat].max():.3f}, mean={X_test[feat].mean():.3f}, std={X_test[feat].std():.3f}"
        )
...
def plot_case_study_performance(
    results_train_normal, results_train_neg, results_test_neg_split, output_dir
):
...
def main():
    ...
    results_train_normal = evaluate_model_on_dataset(
        model_info["model"], df_original, model_info["features"], "Training Data (Normal)"
    )

    # Evaluate on negative training data (the part used for training)
    results_train_neg = evaluate_model_on_dataset(
        model_info["model"], df_neg_train, model_info["features"], "Training Data (Negative)"
    )
    ...
    results_test_neg_split = evaluate_model_on_dataset(
        model_info["model"], df_neg_test, model_info["features"], "Negative Test Data (Split)"
    )
