...
def calculate_monomer_similarity(
    mon1_smiles: str, mon2_smiles: str, query_mon1: str, query_mon2: str
) -> float:
...
def calculate_solvent_similarity_to_set(
    target_solvent_smiles: str, solvent_set_smiles: list
) -> float:
...
def main():
    ...
    case_agreement = (
        df_case["agreement"].tolist() if "agreement" in df_case.columns else [True] * len(df_case)
    )
    ...
    case_solvent_smiles_list = [
        SOLVENT_SMILES[s] for s in original_case_solvents if s in SOLVENT_SMILES
    ]
    ...
    for idx, row in df_train_clean.iterrows():
        ...
        mon_sim = calculate_monomer_similarity(
            row["monomer1_smiles"], row["monomer2_smiles"], MONOMER1_SMILES, MONOMER2_SMILES
        )
        ...
        sol_sim = calculate_solvent_similarity_to_set(
            row["solvent_smiles"], case_solvent_smiles_list
        )
        ...
        for case_solvent in original_case_solvents:
            if case_solvent in SOLVENT_SMILES:
                sim = calculate_tanimoto_similarity(
                    row["solvent_smiles"], SOLVENT_SMILES[case_solvent]
                )
    ...
    for solvent_idx, case_solvent in enumerate(original_case_solvents):
        ...
        matching_indices = [
            i for i, assign in enumerate(all_solvent_assignments) if assign == solvent_idx
        ]
    ...
    all_solvent_assignments_remapped = [
        old_to_new_idx[assign] for assign in all_solvent_assignments
    ]
    ...
    for new_solvent_idx, case_solvent in enumerate(case_solvents_ordered):
        ...
        matching_indices = [
            i
            for i, assign in enumerate(all_solvent_assignments_remapped)
            if assign == new_solvent_idx
        ]
    ...
    ax.plot([-1.2, -1.2], [0.0, grid_top], color="black", linewidth=1.5, alpha=0.7, zorder=2)
    ...
    legend_elements.append(
        Line2D(
            ...
            markeredgecolor="black",
            markeredgewidth=1.2,
            label="No model/lookup agreement (faded)",
            linestyle="None",
            alpha=0.28,
        )
    )
    ...
    class_labels = {0: "Alternating", 1: "Random", 2: "Gradient"}
    ...
    model_correct = sum(
        1 for i in range(n_total) if reordered_minsk_classes[i] == reordered_model_classes[i]
    )

    print(f"\nPerformance of final model on all {n_total} case-study solvents:")
    print(f"  Correct: {model_correct}/{n_total} " f"({model_correct / n_total * 100:.1f}%)")
