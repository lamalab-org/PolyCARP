...
try:
    from copol_prediction.analysis.plot_config import HEATMAP_CMAP, setup_plot_style
except ImportError:
...
# ---------------------------------------------------------------------------
def parse_args():
    parser = argparse.ArgumentParser(description="Sweep filter combinations for the voting model")
    parser.add_argument(
        "--output-dir",
        type=str,
    ...
    parser.add_argument(
        "--plot-only", action="store_true", help="Skip training, re-plot from saved results CSV"
    )
...
def apply_cleaning_filters(df, remove_specialized, apply_poly_filter, set_name="Data"):
...
def main():
    ...
    if args.plot_only:
        ...
        if not os.path.exists(csv_path):
            ...
            legacy_csv = os.path.join(
                _PROJECT_ROOT, "artifacts", "experiments_voting", "sweep_results.csv"
            )
    ...
    for i, (spec, poly, aug) in enumerate(xgb_keys, 1):
        ...
        class_counts = (
            pd.Series(df_train_xgb["r_product_class"].astype(int).values)
            .value_counts()
            .sort_index()
        )
    ...
    for idx, (spec, poly, aug) in enumerate(search_space, 1):
        run_name = f"spec{int(spec)}_poly{int(poly)}" f"_aug{int(aug)}"
    ...
    print(f"\n{'Run':<40} {'Macro Acc':>10} {'Macro Prec':>11} " f"{'Coverage':>9} {'Voting':>7}")
    print("-" * 82)
    for _, r in results_sorted.head(10).iterrows():
        print(
            f"{r['run_name']:<40} {r['macro_accuracy']:>10.4f} "
