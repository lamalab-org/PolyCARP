...
sys.path.insert(0, os.path.join(_script_dir, "../../02-reactivity-prediction/copol_prediction"))
sys.path.insert(0, os.path.join(_script_dir, "../../02-reactivity-prediction"))
...
def select_by_percentile(df, percentile):
    ...
    selected = df[df["importance_mean"] >= threshold].copy()
    print(
        f"Selected {len(selected)} feature groups (top {percentile}th percentile, threshold={threshold:.6f})"
    )
    return selected
...
def select_by_cumulative(df, cumulative_threshold):
    ...
    print(
        f"Selected {len(selected)} feature groups (cumulative importance: {actual_cumulative:.1%})"
    )
    return selected
...
def main():
    ...
    for i, (_, row) in enumerate(selected_df.iterrows(), 1):
        ...
        if pd.notna(features_str):
            ...
            if n_features == 1:
                ...
                print(
                    f"  {i:2d}. {group_label:<45} (importance: {importance:.6f}, {n_features} features)"
                )
    ...
    print(
        f"Reduction: {len(original_features) - len(selected_features)} features ({100 * (1 - len(selected_features)/len(original_features)):.1f}% reduction)"
    )
    ...
    save_selected_features(
        selected_features, selected_df, output_path, metadata, expand_groups=args.expand_groups
    )
