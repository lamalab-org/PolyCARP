...
# ---------------------------------------------------------------------------
def parse_args():
    parser = argparse.ArgumentParser(description="Reaction conditions comparison: with vs without")
...
def load_data():
    ...
    """
    copol_dir = os.path.join(_PROJECT_ROOT, "02-reactivity-prediction", "copol_prediction")
    ...
    if os.path.exists(neg_path):
        ...
        print(
            f"  Negative data available: {len(df_neg)} samples (NOT used, matching final model config)"
        )
...
def _print_condition_variation_pairs(
    ...
    *,
    ...
    eligible = summary[
        (summary["n_r"] >= 2) & (summary["n_cond"] >= 2) & (summary["n_class"] >= 2)
    ].copy()
    if len(eligible) == 0:
        print(
            "  No monomer-pair groups with >=2 (r1,r2), >=2 condition signatures, and >=2 classes."
        )
        return
    ...
    for _, row in pick.iterrows():
        ...
        sort_cols = [
            c
            for c in ["constant_1", "constant_2", "temperature", "solvent_smiles", "method"]
...
def _pair_class_stats_table(df_eval_special: pd.DataFrame) -> pd.DataFrame:
    ...
    out = out.sort_values(["n_rows", "n_class", "n_r", "n_cond"], ascending=False).reset_index(
        drop=True
    )
    return out
...
def plot_top_pairs_true_vs_pred(
    *,
    ...
    for i, key in enumerate(top_keys):
        ...
        ax.scatter(
            x_base + x_off_with + jitter,
            yw,
            s=s,
            alpha=a,
            color=color_with,
            label="With cond." if i == 0 else None,
        )
        ax.scatter(
            x_base + x_off_wo + jitter,
            ywo,
            s=s,
            alpha=a,
            color=color_wo,
            label="Without cond." if i == 0 else None,
        )
    ...
    try:
        ...
        df_show = df_show.sort_values(
            ["pair_rank", "temperature", "solvent_smiles"], ascending=True
        )
...
# ---------------------------------------------------------------------------
def compute_lookup_predictions_expanding(
    df_test, df_train, y_train, fp_dict, *, monomer_only: bool = False
):
...
def evaluate_voting(xgb_pred, lookup_pred, y_true, label):
    ...
    print(
        f"    Agree: {n_agree}  |  Lookup abstain (→XGBoost): {n_abstain}  "
    ...
    base.update(
        {
            ...
            "pair_balanced_accuracy_std": (
                float(np.std(per_pair_scores)) if per_pair_scores.size else 0.0
            ),
        }
    )
    print(
        f"    Within-pair balanced accuracy (mean over pairs): {pair_score:.4f}  (n_pairs={base['n_pairs']})"
    )
    return base
...
def create_comparison_plot(res_full, res_no_cond, output_dir):
    ...
    v_full = [
        res_full.get("pair_balanced_accuracy", res_full["macro_accuracy"]),
        res_full["macro_precision"],
    ]
    v_no = [
        res_no_cond.get("pair_balanced_accuracy", res_no_cond["macro_accuracy"]),
        res_no_cond["macro_precision"],
    ]
    x = np.arange(len(labels_a))

    b1 = ax1.bar(x - w / 2, v_full, w, label="With Reaction\nConditions", color=c1, alpha=alpha)
    b2 = ax1.bar(x + w / 2, v_no, w, label="Without Reaction\nConditions", color=c2, alpha=alpha)
    for bars in [b1, b2]:
        for bar in bars:
            h = bar.get_height()
            ax1.text(
    ...
    for vals, off in [(res_full["per_class_acc"], -w / 2), (res_no_cond["per_class_acc"], w / 2)]:
        for i, v in enumerate(vals):
            ax2.text(i + off, v + 0.03, f"{v:.2f}", ha="center", va="bottom", fontsize=5)
    ...
    for i, pct in enumerate(covs):
        ax3.text(i, pct + max(covs) * 0.02, f"{pct:.1f}%", ha="center", va="bottom", fontsize=7)
...
def main():
    ...
    final_model_path = os.path.join(
        _PROJECT_ROOT, "02-reactivity-prediction", "copol_prediction", "artifacts", "model_bundle"
    )
    training_config = {}
    try:
        ...
        print(
            "  Using defaults for THIS experiment: remove_specialized=False, use_augmentation=False, add_negative_data=False"
        )
        training_config = {
    ...
    if training_config.get("remove_specialized", False):
        if "specialized_filter" in df_lookup_pool.columns:
            ...
            print(
                f"  Lookup pool: Applied specialized filter ({before} -> {len(df_lookup_pool)} samples)"
            )
        else:
            print(
                "  Lookup pool: Warning: 'specialized_filter' column not found; cannot apply specialized filter"
            )
    ...
    if args.subset == "condition_variation":
        ...
        if "reaction_id" in df_eval_special_raw.columns:
            ...
            df_eval_special_raw = df_eval_special_raw.drop_duplicates(
                subset=["reaction_id"], keep="first"
            )
        ...
        print(
            f"  Evaluation split (special subset): condition_variation ({len(df_eval_special)} rows)"
        )
        print(
            f"  Excluded (class change w/o temp+solvent change): {n_excluded_nts} rows (from eval split)"
        )
    else:
    ...
    if df_eval_special is not None and (
        int(args.remove_per_multisolvent_group) > 0 or int(args.keep_per_multisolvent_group) > 0
    ):
    ...
    # Feature sets
    all_features = [c for c in prediction_utils.feature_columns if c in df_train.columns]
    no_cond_features = [f for f in all_features if f not in REACTION_CONDITION_FEATURES]
    ...
    print(f"  Lookup (with cond.): {len(lookup_pred)} predictions  " f"(abstained on {n_abstain})")
    ...
    print(
        f"  Lookup (without cond.): {len(lookup_pred_no_cond)} predictions  "
    ...
    if do_train:
        ...
        print(
            f"  CV score (from previous run): {cv_full:.4f}"
            if not np.isnan(cv_full)
            else "  CV score: (not available)"
        )
    ...
    if do_train:
        ...
        print(
            f"  CV score (from previous run): {cv_no_cond:.4f}"
            if not np.isnan(cv_no_cond)
            else "  CV score: (not available)"
        )
    ...
    if special_mask is not None:
        if df_eval_special is None or len(df_eval_special) == 0:
            ...
            print(
                f"  (original special subset was : {int(special_mask.sum())} rows before dedup/pruning)"
            )
            ...
            res_no_cond = evaluate_voting_within_pair(
                xgb_pred_no_cond[idx],
                lookup_pred_no_cond[idx],
                y_true,
                pair_keys,
            ...
            pred_with_special, _ = _voting_output(xgb_pred_full[idx], lookup_pred[idx])
    ...
    for key, label in [
        ("pair_balanced_accuracy", "Within-pair balanced acc"),
    ...
    print(
        f"{'CV score (XGBoost)':<25} {cv_full:>12.4f} {cv_no_cond:>14.4f} "
        f"{cv_no_cond - cv_full:>+10.4f}"
    )
    print(
        f"{'Num features (XGBoost)':<25} {len(all_features):>12d} " f"{len(no_cond_features):>14d}"
    )
