...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description="Train model with baseline predictions as feature")
    parser.add_argument(
        "--output-dir", type=str, default="results", help="Directory to save model bundle"
    )
    parser.add_argument(
...
def prepare_data_with_baseline_feature(config):
    ...
    print(
        f"  Fingerprint cache size: {len(fp_dict)} (valid: {len([v for v in fp_dict.values() if v is not None])})"
    )
...
def main():
    ...
    config = {
        ...
        "augmentation_samples": (
            ...
            else final_config["augmentation_samples"]
        ),
