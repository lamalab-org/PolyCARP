...
def parse_args():
    parser = argparse.ArgumentParser(description="Compare models for final model selection")
    parser.add_argument(
        "--base-model-path",
        type=str,
        default="../../02-reactivity-prediction/copol_prediction/artifacts/model_bundle",
    ...
    parser.add_argument(
        "--output-dir", type=str, default="comparison", help="Output directory for plots"
    )
...
def load_and_validate_data():
    ...
    if has_overlap:
        ...
        print(
            f"    Total:      {len(train_ids) + len(val_ids) + len(test_ids)} unique reaction_ids"
        )
...
def filter_training_data_for_lookup(df_train, remove_specialized=False):
    ...
    if remove_specialized:
        ...
        spec_path = os.path.join(
            project_root,
            "02-reactivity-prediction/copol_prediction/filter/llm_specialized_filter/classified_output.csv",
        )
            ...
            if {"specialized_filter", "reaction_id"}.issubset(df_spec.columns):
                ...
                if "llm_specialized_filter" in df_filtered.columns:
                    ...
                    df_filtered = df_filtered[
                        df_filtered["llm_specialized_filter"] != "specialized"
                    ]
                    removed_count = before_count - len(df_filtered)
                    if removed_count > 0:
                        print(
                            f"  Removed {removed_count} specialized datapoints from lookup training set"
                        )
...
def train_xgboost_no_filters(df_train, features, random_state=42, n_iter=15):
    ...
    if "r1r2_variant_source" in df_train.columns:
        ...
        print(
            f"  DEBUG: After filtering (after: {len(df_train)}, removed: {before - len(df_train)})"
        )
    else:
    ...
    print(
        f"  DEBUG: Class distribution: {pd.Series(y_train).value_counts().sort_index().to_dict()}"
    )
...
def get_xgboost_predictions(model_path, df_test, use_no_filters_model=False, df_train=None):
    ...
    if use_no_filters_model and df_train is not None:
        ...
        if "r1r2_variant_source" in df_train_clean.columns:
            ...
            print(
                f"  DEBUG: Found 'r1r2_variant_source' column, value counts: {df_train_clean['r1r2_variant_source'].value_counts().to_dict()}"
            )
            df_train_clean = df_train_clean[
                df_train_clean["r1r2_variant_source"] == "original"
            ].copy()
            if len(df_train_clean) < before:
                print(
                    f"  Removed {before - len(df_train_clean)} augmented rows (keeping only originals)"
                )
            else:
                print(f"  DEBUG: No augmented rows found (all rows are 'original')")
        else:
            print(
                f"  DEBUG: 'r1r2_variant_source' column NOT found - assuming all data is original"
            )
        ...
        model, features = train_xgboost_no_filters(
            df_train_clean, features, random_state=42, n_iter=15
        )
...
def train_xgboost_plus_lookup_no_filters(df_train, base_features, random_state=42, n_iter=15):
    ...
    if "r1r2_variant_source" in df_train_clean.columns:
        ...
        print(f"  DEBUG: Filtering augmented rows (before: {before})")
        print(
            f"  DEBUG: Value counts: {df_train_clean['r1r2_variant_source'].value_counts().to_dict()}"
        )
    ...
    model, _ = train_xgboost_no_filters(
        df_train_ext, all_features, random_state=random_state, n_iter=n_iter
    )
    return model, all_features


def get_xgboost_plus_lookup_predictions(
    model_path, df_test, df_train, remove_specialized=False, use_no_filters_model=False
):
    """
    ...
    if use_no_filters_model:
        ...
        model, all_features = train_xgboost_plus_lookup_no_filters(
            df_train, base_features, random_state=42, n_iter=15
        )
        ...
        y_train = df_train_filtered["r_product_class"].astype(int).values
        feature_cols = [
            c for c in prediction_utils.feature_columns if c in df_train_filtered.columns
        ]
        baseline_pred, baseline_sim = compute_naive_baseline_predictions_with_similarity(
            df_test, df_train_filtered, y_train, feature_cols
        )
...
def compute_voting(pred_a, pred_b, name_a, name_b, y_true):
    ...
    for cls in sorted(np.unique(y_true)):
        ...
        print(
            f"  {get_class_label(cls, 'short'):<14} {n_cls_agree:>7} {n_cls_disagree:>9} {pct:>7.1f}%"
        )
    print("=" * 60)
...
def plot_confidence_distributions(all_confidences, all_predictions, y_true, output_dir):
    ...
    fig, axes = plt.subplots(
        1, n_models, figsize=(TWO_COL_WIDTH_INCH * n_models / 3, 3.5), sharey=True
    )
    if n_models == 1:
        axes = [axes]
    ...
    for ext in ["png", "pdf"]:
        plt.savefig(
            os.path.join(output_dir, f"confidence_distributions.{ext}"),
            dpi=300,
            bbox_inches="tight",
        )
    plt.close()
...
def plot_confidence_threshold_sweep(
    all_predictions, all_confidences, y_true, thresholds, output_dir, reaction_ids=None
):
    """
    ...
    for name in names:
        ...
        print(
            f"    Initial: conf shape={conf.shape}, pred shape={pred.shape}, y_true shape={y_true.shape}"
        )
        print(
            f"    conf: min={np.nanmin(conf):.3f}, max={np.nanmax(conf):.3f}, "
        ...
        print(
            f"    After filtering valid_base: conf_v shape={conf_v.shape}, "
            f"conf_v NaN count={np.isnan(conf_v).sum()}/{len(conf_v)}"
        )
        print(
            f"    After filtering valid_base: pred_v shape={pred_v.shape}, "
            f"pred_v NaN count={np.isnan(pred_v).sum()}/{len(pred_v)}"
        )
        print(
            f"    After filtering valid_base: yt_v shape={yt_v.shape}, "
        ...
        print(
            f"    After removing NaN conf: conf_v shape={conf_v.shape}, "
        ...
        if len(conf_v) != len(pred_v) or len(pred_v) != len(yt_v):
            print(
                f"    ERROR: Length mismatch! conf_v={len(conf_v)}, pred_v={len(pred_v)}, yt_v={len(yt_v)}"
            )
        ...
        for t in thresholds:
            ...
            if n_kept > 0:
                ...
                if reaction_ids_v is not None:
                    ...
                    df_kept = pd.DataFrame({"reaction_id": reaction_ids_kept, "class": yt_kept})
                ...
                if len(yt_kept) != len(yp_kept):
                    print(
                        f"      ERROR: Length mismatch! yt_kept={len(yt_kept)}, yp_kept={len(yp_kept)}"
                    )
                ...
                print(
                    f"      t={t:.1f}: n_kept={n_kept:4d}, macro_accuracy={acc:.4f}, "
        ...
        if valid_accs:
            print(
                f"    Summary: macro_accuracy range [{min(valid_accs):.3f}, {max(valid_accs):.3f}], "
    ...
    fig.legend(
        names, fontsize=7, loc="lower center", bbox_to_anchor=(0.5, -0.05), ncol=3, frameon=False
    )
    ...
    for ext in ["png", "pdf"]:
        plt.savefig(
            os.path.join(output_dir, f"confidence_threshold_sweep.{ext}"),
            dpi=300,
            bbox_inches="tight",
        )
    plt.close()
...
def plot_confidence_threshold_sweep_full_range(
    all_predictions, all_confidences, y_true, thresholds, output_dir, reaction_ids=None
):
    """
    ...
    for ext in ["png", "pdf"]:
        plt.savefig(
            os.path.join(output_dir, f"confidence_threshold_sweep_full_range.{ext}"),
            dpi=300,
            bbox_inches="tight",
        )
    plt.close()
...
def plot_confusion_matrices(all_predictions, y_true, output_dir):
    ...
    for ext in ["png", "pdf"]:
        plt.savefig(
            os.path.join(output_dir, f"confusion_matrices.{ext}"), dpi=300, bbox_inches="tight"
        )
    plt.close()
...
def plot_prediction_coverage(voting_stats_list, output_dir):
    ...
    bars_made = ax.bar(
        ...
        made,
        width,
        label="Predicted (agreement)",
        color=COMPARISON_COLORS.get("correct", "#2266ac"),
        alpha=0.85,
        edgecolor="black",
        linewidth=0.4,
    )
    bars_lost = ax.bar(
        ...
        width,
        bottom=made,
        label="Not predicted (disagreement)",
        color=COMPARISON_COLORS.get("incorrect", "#920506"),
        alpha=0.85,
        edgecolor="black",
        linewidth=0.4,
    )
    ...
    for ext in ["png", "pdf"]:
        plt.savefig(
            os.path.join(output_dir, f"prediction_coverage.{ext}"), dpi=300, bbox_inches="tight"
        )
    plt.close()
...
def main():
    ...
    try:
        ...
        try:
            ...
            if (
                remove_specialized_base != remove_specialized_lf
                or use_augmentation_base != use_augmentation_lf
                or negative_data_base != negative_data_lf
            ):
                print(f"\n  ⚠️  WARNING: Model training filters differ!")
                print(
                    f"     Base model and Lookup Features model were trained with different filters."
                )
    ...
    print("=" * 60)
    lookup_pred, lookup_sim = get_lookup_predictions(
        df_val, df_train, remove_specialized=remove_specialized
    )
    ...
    try:
        xgb_lf_pred, xgb_lf_conf, xgb_lf_proba = get_xgboost_plus_lookup_predictions(
            args.baseline_feature_model_path,
            df_val,
            df_train,
    ...
    if lookup_pred is not None and xgb_pred is not None:
        v_pred, v_mask, v_stats = compute_voting(lookup_pred, xgb_pred, "Lookup", "XGBoost", y_true)
    ...
    print("=" * 60)
    print(
        "Training models with basic dataset (no augmentation, no specialized filter, no negative data)..."
    )
    ...
    lookup_pred_no_filters, lookup_sim_no_filters = get_lookup_predictions(
        df_val, df_train_clean, remove_specialized=False
    )
    ...
    try:
        xgb_lf_pred_no_filters, xgb_lf_conf_no_filters, xgb_lf_proba_no_filters = (
            get_xgboost_plus_lookup_predictions(
                args.baseline_feature_model_path,
                df_val,
                df_train_clean,
                remove_specialized=False,
                use_no_filters_model=True,
            )
        )
