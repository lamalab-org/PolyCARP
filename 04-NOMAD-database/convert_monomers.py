...
``02-reactivity-prediction/copol_prediction/processed_data.csv``): for every distinct
``monomer1_smiles``/``monomer2_smiles`` we

  1. look up the precomputed quantum-feature JSON under
     ``02-reactivity-prediction/copol_prediction/api/molecule_properties/<md5(smiles)>.json``
...
def archive_monomer_universe(
    ...
    if stubbed_smiles:
        print(
            ...
            "feature file); rerun 02-reactivity-prediction/copol_prediction/monomer_feature_calculation.py "
