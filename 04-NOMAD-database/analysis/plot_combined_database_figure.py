...
sys.path.insert(0, str(PROJECT_ROOT / "02-reactivity-prediction" / "copol_prediction" / "analysis"))

from plot_config import SEQUENTIAL_COLORS, TWO_COL_WIDTH_INCH, setup_plot_style  # noqa: E402

DATA_PATH = PROJECT_ROOT / "02-reactivity-prediction" / "copol_prediction" / "processed_data.csv"
...
def _count_raw_llm_extractions() -> dict[str, int]:
    ...
    Source:
    `01-data-extraction/artifacts/llm/extractions/model_output_GPT4-o/*.json`
    """
    base = (
        PROJECT_ROOT
        / "01-data-extraction"
...
def _count_artifacts_extracted_reactions_csv() -> dict[str, int]:
    ...
    Source:
    `01-data-extraction/artifacts/datasets/extracted_reactions.csv`
    """
    p = PROJECT_ROOT / "01-data-extraction" / "artifacts" / "datasets" / "extracted_reactions.csv"
    if not p.exists():
